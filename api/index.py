...
CA_SPOTS = [
    ...
    {"key": "avalanche",      "name": "Avalanche",            "lat": 32.7544, "lng": -117.2534, "zone": "OB / Point Loma",  "note": "Beach break (NOT a reef) — needs SW/W swell 197-307deg", "state": "CA"},
...
FL_SPOTS = [
    {"key": "romano",         "name": "Romano Beachfront Park", "lat": 29.2724, "lng": -81.0590, "zone": "Palm Coast S",   "note": "Beach break - S Palm Coast", "state": "FL"},
    {"key": "granada",        "name": "Granada",                "lat": 29.2853, "lng": -81.0572, "zone": "Palm Coast S",   "note": "Beach break", "state": "FL"},
    {"key": "bicentennial",   "name": "Bicentennial Park",      "lat": 29.2940, "lng": -81.0555, "zone": "Palm Coast",     "note": "Beach break", "state": "FL"},
    {"key": "tom_renick",     "name": "Tom Renick Park",        "lat": 29.3098, "lng": -81.0530, "zone": "Palm Coast",     "note": "Beach break", "state": "FL"},
    {"key": "main_street_fl", "name": "Main Street",            "lat": 29.4735, "lng": -81.1278, "zone": "Flagler Beach",  "note": "Beach break - town beach", "state": "FL"},
    {"key": "flagler_pier",   "name": "Flagler Beach Pier",     "lat": 29.4728, "lng": -81.1283, "zone": "Flagler Beach",  "note": "Pier break - best at mid tide", "state": "FL"},
    {"key": "marineland",     "name": "Marineland",             "lat": 29.6648, "lng": -81.2132, "zone": "Marineland",     "note": "Beach break - N of Flagler", "state": "FL"},
]
...
STATE_CONFIG = {
    "CA": {
        ...
        "system_prompt": """You are a surf forecast AI agent for an experienced San Diego surfer who surfs ~90% of days and does dawn patrol.
...
- Avalanche: BEACH BREAK (NOT a reef) — needs SW/W swell 197-307deg; north/NW swell misses it
...
SESSION FORECAST - [Day, Month Date, Time Window]
Conditions: [1-sentence cross-referenced swell/wind summary]

RANKED SPOTS
1. [Spot name] [score out of 5, half-points ok e.g. 3.5]  [height ft] | [period]s | [swell dir]deg | wind [speed]mph [dir]deg | tide [ft]
...
Star ratings are ABSOLUTE, not relative to the other checked spots. Rate each spot on its own merits. Half-point increments are allowed (e.g. 3.5, 4.5):
- 5: Pumping — 4ft+ with 12s+ groundswell, offshore winds, good tide. Genuinely great day.
- 4-4.5: Good — 3-4ft, 10s+ period, light wind, tide working. Worth getting up early.
- 3-3.5: Decent — 2-3ft, 8-10s, manageable conditions. Fun but not epic.
- 2-2.5: Marginal — under 2ft, short period, or onshore wind. Surfable but barely worth it.
- 1-1.5: Poor — flat, blown out, or wrong swell direction. Skip it.
If all spots are bad, give them all low scores. Do NOT give 5 stars just because it's the best of a bad bunch.
Convert wave height to feet (1m = 3.28ft). Be direct and specific.""",
    ...
    "FL": {
        ...
        "buoys": [
            {"id": "41009", "name": "Canaveral 20NM E", "note": "main offshore buoy for NE FL"},
            {"id": "41047", "name": "NE of Bahamas",    "note": "tracks long-range swell approaching from E/NE"},
        ],
        "system_prompt": """You are a surf forecast AI agent for an experienced surfer on the NE Florida Atlantic coast (Palm Coast / Flagler Beach area).
...
LOCAL KNOWLEDGE:
- Swell: E/NE swell is best for this stretch of coast (comes in clean and direct)
- Wind: offshore = westerly (270-300 deg); onshore = easterly (90 deg)
- Flagler Beach Pier: best at mid tide, can close out at high, gets shallow at low
- Tidal range is significant (~4ft) — tide timing matters more here than in CA
- Hurricane and nor'easter swells produce the best surf (fall/winter)
- Summer is typically small; E/SE wind swell is common but short-period
- Wave period: >10s = quality swell; 7-10s = moderate; <7s = wind chop
- Good height: 0.5-1.5m (1.5-5ft). Under 0.3m = flat.
...
Star ratings are ABSOLUTE, not relative to the other checked spots. Rate each spot on its own merits. Half-point increments are allowed (e.g. 3.5, 4.5):
- 5: Pumping — 4ft+ with 10s+ groundswell, offshore winds, good tide. Genuinely great day.
- 4-4.5: Good — 3-4ft, 8s+ period, light wind, tide working. Worth getting up early.
- 3-3.5: Decent — 2-3ft, manageable conditions. Fun but not epic.
- 2-2.5: Marginal — under 2ft, short period, or onshore wind. Surfable but barely worth it.
- 1-1.5: Poor — flat, blown out, or wrong swell direction. Skip it.
If all spots are bad, give them all low scores. Do NOT give 5 stars just because it's the best of a bad bunch.
Convert wave height to feet (1m = 3.28ft). Be direct and specific.""",
...
def fetch_openmeteo(spot, target_date, hour_range, forecast_days):
    ...
    if not lines:
        return f"{spot['name']}: no data for requested window"
...
def call_claude(openmeteo_block, buoy_block, tide_block, time_label, system_prompt, spot_names, feedback=None):
    ...
    if feedback:
        ...
        feedback_block = f"SURFER FEEDBACK FROM PAST FORECASTS (use this to calibrate your predictions):\n{notes}\n\n"

    spots_line = f"CHECKED SPOTS ({len(spot_names)}): {', '.join(spot_names)}\nOnly rank these specific spots — do not mention or invent any others.\n\n"
...
def get_spots():
    ...
    spots = CA_SPOTS if state == "CA" else FL_SPOTS
...
def get_forecast():
    try:
        ...
        spot_pool = CA_SPOTS if state == "CA" else FL_SPOTS
        if keys:
