...
- Wind: offshore = easterly (~90°); onshore = westerly (~270°); dawn often glassy in summer
...
# ---------------------------------------------------------------------------
# Fetch helpers (stdlib only — no requests on Vercel serverless)
# ---------------------------------------------------------------------------
    ...
    for buoy in NOAA_BUOYS:
        ...
        try:
            ...
            for row in rows:
                ...
                readings.append(
                    f"  {mo}/{dy} {hh}:{mm_t}Z — "
                    f"{ft:.1f}ft @ {dpd}s from {mwd}° | wind {wspd}mph/{wdir_b}°"
                )
            if readings:
                results.append(
...
def fetch_tides(target_date):
    ...
    if any(l <= -1.2 for l in lows):
        warning = f"\n  ⚠️  EXTREME LOW TIDE ({min(lows):+.2f} ft) — skip Avalanche south side, use OB pier north sandbar"
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if not spots:
            self._json(400, {"error": "No valid spots provided"})
            return
